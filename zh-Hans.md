...
Phrabbit 是一款用于外语听力练习的 A/B 区间循环应用。你可以把音频文件、视频文件、音乐库曲目、播客和 YouTube 链接作为学习材料添加进来，然后反复听指定片段，或跟读并录下自己的声音。应用还支持语音转文字字幕、单词本、跟读录音和学习记录。
...
13. [播客](#13-播客)
14. [学习记录与设置](#14-学习记录与设置)
15. [免费版 vs 高级版](#15-免费版-vs-高级版)
16. [常见问题](#16-常见问题)
...
### 2-2. 收藏与文件夹
长按资料库项目可以打开上下文菜单。选择 **Add to Favorites** 可将项目加入收藏，选择 **Move to Folder…** 可整理项目。至少有一个收藏项目后，标志下方会出现 **All / Favorites** 筛选标签；选择 **Favorites** 会显示整个资料库中的收藏项目，不受文件夹位置影响。
...
### 2-4. 删除
将项目向左滑动后点按 **Delete**。为了避免普通滚动误删项目，整行滑动删除已关闭。删除后，与该材料关联的波形缓存、书签、字幕和已保存的跟读录音也会一起清理。如果该项目有已保存录音，删除前会显示确认窗口。
...
当书签和录音都存在时，可以用 **Bookmarks / My Recordings** 切换分别查看。书签达到 2 个以上时，可能会出现 **Play All** 菜单。它可以按顺序播放多个书签，并将每个书签设置为重复 1、2 或 3 次。向左滑动书签会显示 **Rename** 和 **Delete**，点按后执行相应操作。
...
- 使用时间标尺精确移动长视频
- 支持全屏练习
- 按 YouTube 链接保存书签和跟读录音。书签会立即以自动名称保存，可从保存提示或已保存项目中选择 **Rename** 修改名称。
...
存放在设备或 iCloud Drive 中的视频文件同样可以作为学习材料。视频也能使用与音频相同的 A/B 区间循环、跟读和书签功能，字幕则通过导入单独的字幕文件来使用。
...
- A 和 B 都设置好后，会出现区间信息栏，其中有区间显示、微调按钮、场景转写按钮、跟读用的麦克风按钮，以及清除区间的关闭按钮。
- 再往下是播放控制：**A-B**、循环次数、播放速度、跟读间隔、后退/前进 5 秒、播放/暂停，以及保存书签的 **Save**。
- 最下方会列出已保存的 **Bookmarks** 和 **My Recordings**。两者都存在时，可以用切换控件分别查看。
...
字幕显示在视频下方。点按字幕可以隐藏，长按可以直接修改当前这句。以非 UTF-8 编码保存的字幕文件可能无法导入，请另存为 UTF-8 文本后重试。

点按字幕句子上的加号按钮，可以把单词保存到单词本。单词本是高级版功能。
...
### 11-7. 重命名与删除
视频标题最初显示为文件名。在资料库中长按视频并选择 **Rename Video** 即可修改。

向左滑动视频并删除时，会清理 Phrabbit 中的项目信息以及字幕、书签和已保存的跟读录音。**原始视频文件不会被删除。** 如果存在已保存的录音，删除前会先显示确认框。
