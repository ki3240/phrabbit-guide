...
删除后，与该材料关联的波形缓存、书签、字幕和已保存的跟读录音也会一起清理。如果该项目有已保存录音，删除前会显示确认窗口。删除文件夹时，可以选择只删除文件夹，或连同其中的材料一起删除。
...
当书签和录音都存在时，可以用 **Bookmarks / My Recordings** 切换分别查看。书签达到 2 个以上时，可能会出现 **Play All** 菜单。它可以按顺序播放多个书签，并将每个书签设置为重复 1、2 或 3 次。
...
### 8-2. 编辑字幕
点按字幕卡片的编辑按钮可以修改文本。如果修改的是 STT 生成的字幕，原文也会一起显示，必要时可用 **Reset** 恢复。在编辑页面中，也可以用 **Recognize again** 只重新识别当前句子的区间，再用 **Put in editor** 将预览结果放入编辑框。只有点按顶部的 **Save** 后才会保存。
...
- 使用时间标尺精确移动长视频
- 支持全屏练习
- 按 YouTube 链接保存书签和跟读录音。书签会立即以自动名称保存，可从保存提示，或已保存项目的 **⋯**／长按菜单中选择 **Rename** 修改名称。同一菜单的 **Info** 可查看准确范围、长度和保存日期，也可用 **Delete** 删除。空间足够时，书签卡片也会显示保存日期。
...
在同一菜单中选择 **Delete**，会清理 Phrabbit 中的项目信息以及字幕、书签和已保存的跟读录音。**原始视频文件不会被删除。** 如果存在已保存的录音，删除前会先显示确认框。
