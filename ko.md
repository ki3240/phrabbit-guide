...
Phrabbit은 외국어 듣기 연습을 위한 A/B 구간 반복 앱입니다. 오디오 파일, 동영상 파일, 뮤직 라이브러리, 팟캐스트, YouTube 링크를 학습 자료로 추가하고, 원하는 구간을 반복해서 듣거나 따라 말하며 연습할 수 있습니다. 자막 변환, 단어장, 쉐도잉 녹음, 학습 기록도 함께 사용할 수 있습니다.
...
13. [팟캐스트](#13-팟캐스트)
14. [학습 기록과 설정](#14-학습-기록과-설정)
15. [무료 vs 프리미엄](#15-무료-vs-프리미엄)
...
### 2-2. 즐겨찾기와 폴더
라이브러리 항목을 길게 눌러 컨텍스트 메뉴를 엽니다. **Add to Favorites**로 즐겨찾기에 추가하거나 **Move to Folder…**로 정리할 수 있습니다. 즐겨찾기가 하나 이상 생기면 워드마크 아래에 **All / Favorites** 칩이 나타납니다. **Favorites**를 선택하면 폴더 위치와 관계없이 전체 라이브러리에서 즐겨찾기만 표시됩니다.
...
### 2-4. 삭제
항목을 왼쪽으로 스와이프한 뒤 **Delete**를 누릅니다. 실수로 스크롤하다가 항목이 삭제되지 않도록 전체 스와이프 삭제는 꺼져 있습니다. 삭제하면 해당 자료와 연결된 파형 캐시, 북마크, 자막, 저장된 쉐도잉 녹음이 함께 정리됩니다. 저장된 녹음이 있는 항목은 삭제 전에 확인창이 표시됩니다.
...
자세한 내용은 [13. 팟캐스트](#13-팟캐스트)를 참고하세요.
...
북마크와 녹음이 모두 있으면 **Bookmarks / My Recordings** 전환으로 두 목록을 나누어 볼 수 있습니다. 북마크가 2개 이상 있으면 **Play All** 메뉴가 나타날 수 있습니다. 이 기능은 여러 북마크를 순서대로 재생하고, 각 북마크를 1, 2, 3회씩 반복하도록 설정할 수 있습니다. 북마크를 왼쪽으로 스와이프하면 **Rename**과 **Delete**가 나타나며, 원하는 동작을 눌러 실행합니다.
...
- 전체화면 연습 지원
- YouTube 링크별 북마크와 쉐도잉 녹음 저장. 북마크는 자동 이름으로 즉시 저장되며, 저장 토스트나 저장된 항목에서 **Rename**으로 이름을 바꿀 수 있습니다.
...
기기나 iCloud Drive에 있는 동영상 파일도 학습 자료로 사용할 수 있습니다. 동영상에서도 오디오와 같은 A/B 구간 반복, 쉐도잉, 북마크를 그대로 쓸 수 있고, 자막은 별도의 자막 파일을 불러와서 사용합니다.
...
- A와 B가 모두 설정되면 구간 정보 바가 나타납니다. 구간 표시, 미세 조정 버튼, 구간 자막 생성 버튼, 쉐도잉용 마이크 버튼, 구간을 해제하는 닫기 버튼이 들어 있습니다.
- 그 아래에 재생 컨트롤이 있습니다. **A-B**, 반복 횟수, 재생 속도, 쉐도잉 간격, 5초 뒤/앞 이동, 재생/일시정지, 북마크 저장용 **Save**입니다.
- 맨 아래에는 저장한 **Bookmarks**와 **My Recordings**가 표시됩니다. 둘 다 있을 때는 전환 컨트롤로 두 목록을 나누어 볼 수 있습니다.
...
자막은 영상 아래쪽에 표시됩니다. 자막을 탭하면 숨길 수 있고, 길게 누르면 현재 문장을 바로 수정할 수 있습니다. UTF-8이 아닌 인코딩으로 저장된 자막 파일은 불러오지 못할 수 있으니, UTF-8 텍스트로 저장한 뒤 다시 시도해 주세요.

자막 문장의 플러스 버튼을 누르면 단어를 단어장에 저장할 수 있습니다. 단어장은 프리미엄 기능입니다.
...
### 11-7. 이름 변경과 삭제
동영상 제목은 처음에 파일 이름으로 표시됩니다. 라이브러리에서 동영상을 길게 눌러 **Rename Video**를 선택하면 이름을 바꿀 수 있습니다.

동영상을 왼쪽으로 스와이프해 삭제하면 Phrabbit의 항목 정보와 자막, 북마크, 저장된 쉐도잉 녹음이 정리됩니다. **원본 동영상 파일은 삭제되지 않습니다.** 저장된 녹음이 있으면 삭제 전에 확인창이 표시됩니다.
