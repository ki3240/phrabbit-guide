...
削除すると、その素材に関連する波形キャッシュ、ブックマーク、字幕、保存済みシャドーイング録音も一緒に整理されます。保存済み録音がある項目では、削除前に確認画面が表示されます。フォルダを削除するときは、フォルダだけを削除するか、中の素材も一緒に削除するかを選べます。
...
ブックマークと録音の両方がある場合は、**Bookmarks / My Recordings** の切り替えで一覧を分けて表示します。ブックマークが 2 件以上あると、**Play All** メニューが表示される場合があります。複数のブックマークを順番に再生し、それぞれを 1、2、3 回ずつ繰り返すよう設定できます。
...
### 8-2. 字幕を編集
字幕カードの編集ボタンをタップすると、テキストを修正できます。STT で作成した字幕を編集した場合は元のテキストも表示され、必要に応じて **Reset** で戻せます。編集画面でも **Recognize again** でその文の区間だけを再認識し、プレビューを **Put in editor** で編集欄へ移せます。画面上部の **Save** をタップするまで保存されません。
...
- 全画面練習に対応
- YouTube リンクごとにブックマークとシャドーイング録音を保存。ブックマークは自動名ですぐに保存され、保存トースト、または保存済み項目の **⋯**／長押しメニューから **Rename** で名前を変更できます。同じメニューの **Info** で正確な範囲、長さ、保存日を確認したり、**Delete** で削除したりできます。スペースに余裕がある場合は、ブックマークカードにも保存日が表示されます。
...
同じメニューから **Delete** を選ぶと、Phrabbit 側の項目情報と字幕、ブックマーク、保存済みシャドーイング録音が整理されます。**元の動画ファイルは削除されません。** 保存済み録音がある場合は、削除前に確認画面が表示されます。
